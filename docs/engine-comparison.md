...
## 8. v0.6.0 POC 結果（方式 B）

v0.6.0 で方式 B（Markdig + Open XML SDK）の POC 実装を行い、実用可能であることを確認した。
詳細は `docs/poc-openxml.md` を参照。

### テスト結果サマリ

- 全 13 テストが **Word 不要・CI 実行可能**
- 必須 Markdown 要素（見出し・段落・箇条書き・順序付きリスト・表・改ページ・日本語）を確認
- `DocxConversionTests`（Word COM 方式）と同一 Markdown・同一観点で回帰テストを設計

### POC で確認された優位性

- v0.3.0 の本文消失・箇条書き伝播問題は構造的に発生しない
- 順序付きリストが Word COM 方式では未対応だったが、Open XML 方式で正しく実装できた
- CI での完全自動テストが実現（Word なし環境で passed 偽装なし）

### POC 段階の制約（本実装時に追加が必要）

- インライン書式（太字・斜体）はテキスト抽出のみ（装飾なし）
- ヘッダー・フッター未対応
- 水平線（HR）未対応
- 配布サイズが約 6〜8 MB 増加
...
v0.5.0 の推奨（方式 B を本命候補）を POC 実装で裏付けた。
方式 B への本実装移行を **v0.7.0** 以降の課題として提案する。
...
- 関連ドキュメント:
  ...
  - `docs/poc-openxml.md` — 方式 B の POC 記録（v0.6.0）
