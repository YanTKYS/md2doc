...
- **`src/md2doc/MainForm.cs`**
  ...
  - `MinimumSize` を `760×580` → `760×640` に修正
    - inputSection の最小高さ（~250px）と settingsSection の高さ（~296px）から
      必要クライアント高さを ~570px と算出。580px では境界値となり実機で溢れが確認されたため
      60px の余裕を持たせた値に修正する
...
v1.0.0 の庁内配布に向けて、WinForms 画面の見た目と操作感を整えた。
機能追加はなく、既存の変換処理・設定保存・動作はすべて維持する。

### 変更内容

- **`src/md2doc/MainForm.cs`**
  - 変換エンジン ラジオボタンのラベルを整理
    - "Open XML方式（標準・高速）" / "Word COM方式（互換確認用）"
  - 「変換実行」ボタンを主ボタンとして強調（太字・`MinimumSize=110×34`・横 Padding）
  - 「入力方式」ラベルを inputModePanel 内に統合（inputSection を 8 行 → 7 行に整理）
  - `root` の Padding を 10 → 12 に拡大（全体の余白を確保）
  - 設定セクションを 4 行 → 5 行に拡張し「変換エンジン」を独立 GroupBox へ分離
  - GroupBox の名称を整理
    - "文書フォント設定" → "フォント設定"
    - "オプション" → "文書オプション"
    - 新設: "変換エンジン"
  - 「文書オプション」GroupBox から変換エンジン行を除去（3 行 → 3 行のままだが engine 行削除）
  - 結果ラベルに上マージン（4px）を追加し視覚的な区切りを確保

### 維持される動作

- Open XML 方式・Word COM 方式の変換処理
- フォント・ヘッダー・フッター・見出し番号設定の保存と復元
- 変換後の「Wordファイルを開く」「保存先フォルダを開く」ボタン
- 2 段構成レイアウト（入力セクション伸縮・設定セクション固定）

### 既知の制約

v0.6.2 と同じ。画像・脚注・引用ブロック等は未対応。
