...
v0.5.1 で POC 実装を行い、v0.5.2 で本実装候補として機能拡張、v0.5.3 で UI から選択可能化、
v0.5.4 で実機確認、v0.5.5 でコード品質整備、v0.5.6 でドキュメント整理、
v0.5.7 で Core プロジェクトへ切り出した記録。
...
| ファイル | 説明 |
|----------|------|
| `src/Md2Doc.Core/OpenXmlConverter.cs` | 変換クラス（Word COM 不要・WinForms 非依存、v0.5.7 で `Md2Doc.Core` プロジェクトへ移動） |
| `src/Md2Doc.Core/Md2Doc.Core.csproj` | Open XML 方式ライブラリプロジェクト（`net8.0`、v0.5.7 で追加） |
| `tests/Md2Doc.Tests/OpenXmlConverterTests.cs` | テスト（全 Word 不要、26 テスト、v0.5.7 で Core を直接参照） |
...
WinForms 本体から独立したライブラリプロジェクトへ切り出した。

### プロジェクト構成

| プロジェクト | 役割 | TargetFramework |
|------------|------|----------------|
| `Md2Doc.Core` | Open XML 方式の変換処理（ライブラリ） | `net8.0` |
| `md2doc` (WinForms) | 画面表示・入力取得・設定保存復元・変換方式選択・呼び出し | `net8.0-windows` |
| `Md2Doc.Tests` | 単体・統合テスト | `net8.0-windows` |

### 主な変更点

- `OpenXmlConverter` を `Md2Doc.Core` 名前空間に移動、`public static` に変更
- WinForms 本体（`src/md2doc/`）から `Markdig` / `DocumentFormat.OpenXml` の直接参照を除去
- テストプロジェクトから Open XML 方式テストは Core を直接参照
- ソリューションファイル `md2doc.sln` を追加し、3 プロジェクト一括ビルド可能に

### 効果

- **テスト容易性**: Open XML 方式のテストが WinForms / Microsoft Word いずれにも依存しない
- **保守性**: 変換処理（Core）と画面処理（WinForms）の責務が明確に分離
- **将来拡張性**: 他フロントエンドから参照可能なライブラリ境界が確立

### 対象外

CLI / PowerShell 簡易版 / 複数ファイル一括変換は v1.1.0 以降で必要性が確認された場合に再検討
（`docs/backlog.md` に整理済み）。

---

## 11. 結論

v0.5.1 POC → v0.5.2 本実装候補化 → v0.5.3 UI 統合 → v0.5.4 実機確認 → v0.5.5 品質整備
→ v0.5.6 ドキュメント整理 → v0.5.7 Core 切り出しを経て、Open XML 方式は
**現行 WordInteropConverter の代替として十分な品質と構造に達した** と評価できる。
...
- 変換処理が独立したライブラリプロジェクト（`Md2Doc.Core`）として整備済み
