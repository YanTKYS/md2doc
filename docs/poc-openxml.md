...
v0.5.1 POC → v0.5.2 本実装候補化 → v0.5.3 UI 統合を経て、Open XML 方式は **現行 WordInteropConverter の
...
v0.5.3 からは画面上で両方式を切り替えて実機比較できるようになり、
v1.0.0 に向けた標準候補としての検証フェーズに入った。
正式採用判断・Word COM 廃止判断を **v0.6.0** の課題として提案する。
